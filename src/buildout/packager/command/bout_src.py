...
def copy_python(src_python_dir, build_python_dir):
    ...
    placeholder = os.path.join(src_python_dir, 'Lib', 'site-packages',
                               'README.txt')
...
class bout_src(Command):
    ...
    def run(self):
        ...
        build_dir = os.path.join(cwd, self.build_base,
                                 'buildout-' + get_postfix_name(self.python))
        ...
        if self.include_python:
            executable = os.path.join(build_python_dir,
                                      os.path.basename(self.python))
            log.info("bdist_src including Python Interpreter at '%s'",
                     self.python)
        else:
            if os.path.exists(build_python_dir):
        ...
        for name in ['bin', 'develop-eggs', 'parts', 'buildout_pre.cfg',
                     '.installed.cfg']:
